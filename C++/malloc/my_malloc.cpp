...
void find_free(size_t size, node_t **found, node_t **previous) {
	// TODO
    size_t availableMemory = available_memory();
    node_t *current = heap();

    if (current != NULL && (size <= availableMemory)) {
        // Iterate until found
        while (current != NULL) {
            if (size + sizeof(header_t) <= current->size + sizeof(node_t)) {
                *found = current;
                break;
            } else {
                previous = &current;
...
void split(size_t size, node_t **previous, node_t **free_block, header_t **allocated) {
    ...
    node_t *current = *free_block;
    size_t freeBlockSize = current->size;

    if (size + sizeof(node_t) > freeBlockSize) {
        return;
    }

    size_t memoryNeeded = size + sizeof(header_t);
    *free_block = (node_t *)(((char*)*free_block) + memoryNeeded);
    (*free_block)->size = freeBlockSize - (size + sizeof(node_t));

    if (*previous == NULL) { // is head
        node_t *temp = head;
        head = *free_block;
        *allocated = (header_t *) temp;
    } else {
...
void coalesce(node_t *free_block) {
  // TODO
  size_t size = free_block->size + sizeof(node_t);
  node_t *next = free_block->next;

  if (((char *) free_block) + size == (char *) next) {
      free_block->size = free_block->size + next->size + sizeof(node_t);
...
void my_free(void *allocated) {
  // TODO
  header_t *block = (header_t *)(((char *) allocated) - sizeof(header_t));
  assert(block->magic == MAGIC);
  size_t size = block->size;
  node_t *freed = (node_t *) block;
  freed->size = size;
  freed->next = head;
  head = freed;
  coalesce(freed);
